...
run_once() {
...

  {
    ...
    run_step "AeroGraph external-provider readiness" python scripts/check_aerograph_nonmock_readiness.py
    ...
    run_step "current work status" env PYTHONPATH=scripts python scripts/build_current_work_status.py
