...
EXPERIMENT=${EXPERIMENT:-marinecity_nerfacto_torch_smoke}
TIMESTAMP=${TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}
MAX_ITERS=${MAX_ITERS:-100}
CAMERA_SCALE=${CAMERA_SCALE:-0.5}
IMPORT_AFTER=${IMPORT_AFTER:-1}
...
echo "[nerfstudio-native] start train $(date -Is) gpu=$GPU env=$ENV_NAME data=$DATA_ABS out=$OUT_ABS max_iters=$MAX_ITERS method=nerfacto(torch)" | tee "$TRAIN_LOG"
...
conda run --no-capture-output -n "$ENV_NAME" python - "$RAW_EVAL_JSON" "$METRIC_JSON" "$CONFIG_HOST" "$RENDER_DIR" "$TRAIN_LOG" "$EVAL_LOG" "$MAX_ITERS" "$EXPERIMENT" "$TRAIN_SPLIT_FRACTION" <<'PY'
import json
import sys
...
train_split_fraction = sys.argv[9]
...
row = {
    ...
    "status": f"complete_native_{experiment}",
    ...
    "notes": (
        "Nerfstudio nerfacto torch backend native CUDA 12.8 smoke, "
        f"max_iters={max_iters}, train_split_fraction={train_split_fraction}; "
...
if [[ "$IMPORT_AFTER" == "1" ]]; then
  ...
  echo "[nerfstudio-native] skip import/table refresh because IMPORT_AFTER=$IMPORT_AFTER"
fi

cat > "$MANIFEST" <<JSON
{
  "status": "nerfstudio_nerfacto_torch_smoke_complete",
  "updated_at": "$(date -Is)",
  "gpu": "$GPU",
  "env_name": "$ENV_NAME",
  "data_dir": "$DATA_ABS",
