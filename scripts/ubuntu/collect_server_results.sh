...
python -m evaluation.collect_detector_metrics --detector-root "$DETECTOR_ROOT" --out "$RESULTS_CSV"
python -m evaluation.summarize_seed_results --results-csv "$RESULTS_CSV" --out "$SUMMARY_CSV"
python -m evaluation.statistical_tests --results-csv "$RESULTS_CSV" --out "$PVALUES_CSV"
python -m scripts.build_server_training_dashboard --results-csv "$RESULTS_CSV" --summary-csv "$SUMMARY_CSV" --out "$DASHBOARD"
