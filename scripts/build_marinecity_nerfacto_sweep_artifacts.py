...
def write_tex(rows: list[dict[str, Any]]) -> None:
    ...
    lines = [
        ...
        r"\toprule",
        r"Variant & Iters & PSNR$\uparrow$ & SSIM$\uparrow$ & LPIPS$\downarrow$ & Use \\",
        r"\midrule",
    ]
    for row in ranked:
        lines.append(
            f"{latex_escape(row['variant'])} & {row['iters']} & {fmt(row['PSNR'], 2)} & "
            f"{fmt(row['SSIM'])} & {fmt(row['LPIPS'])} & {latex_escape(row['paper_use'])} \\\\"
        )
