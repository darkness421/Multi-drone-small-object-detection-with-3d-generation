"""Build a transparent internal AeroGraph reviewed-response candidate.

This is a bridge for the no-API/manual external-provider case. It creates valid AeroGraph
...
can be replaced by external-provider or local-LLM responses later without changing the
downstream importer.
"""
...
def main() -> None:
    ...
    parser.add_argument("--out", default="outputs/reasoning/aerograph_manual_responses.reviewed_candidate.jsonl")
        ...
        out_rows.append(
            {
                ...
                "provider_note": (
                    "Internal reviewed AeroGraph candidate; replace with external-provider/local-LLM "
    ...
    print(json.dumps({"status": "aerograph_reviewed_candidate_written", "out": str(out_path), "rows": len(out_rows)}, indent=2))
