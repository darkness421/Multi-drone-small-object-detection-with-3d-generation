...
conda run --no-capture-output -n com3d-ace python -m detectors.train_yolo train --model rtdetr-l.pt --data-yaml configs/detector/visdrone_yolo_data.yaml --epochs 100 --imgsz 1280 --batch 2 --device 0 --seed 42 --project outputs/detectors/server_baselines --name rtdetr-l_visdrone_seed42 2>&1 | tee outputs/logs/server_baselines/rtdetr-l_visdrone_seed42.log
if [[ 1 == 1 ]]; then
...
conda run --no-capture-output -n com3d-ace python -m detectors.train_yolo train --model rtdetr-l.pt --data-yaml configs/detector/visdrone_yolo_data.yaml --epochs 100 --imgsz 1280 --batch 2 --device 0 --seed 2026 --project outputs/detectors/server_baselines --name rtdetr-l_visdrone_seed2026 2>&1 | tee outputs/logs/server_baselines/rtdetr-l_visdrone_seed2026.log
if [[ 1 == 1 ]]; then
