...
Updated: `2026-06-28 06:31:39 KST`
...
- MarineCity integration gate: `marinecity_system_integration_smoke_ready_with_pending_final_gates`; fails `0`, warnings `0`, pending `1`
...
- Fail/warn/pending count: `0` / `0` / `1`
- Detector token count: `117`
- Detector classes: `{'bus': 15, 'car': 65, 'motor': 2, 'pedestrian': 25, 'van': 8, 'truck': 2}`
...
| supp_system | paper/tables/marinecity_crossview_evidence_graph_table.tex | supp | True | ok | Real-Cesium cross-view evidence graph smoke table |
...
| main_system | paper/tables/marinecity_3d_completion_results_table.tex | main_or_supp_smoke | True | ok | Compact neural 3D runner-family smoke metric table |
...
| pending_reasoner | paper/tables/aerograph_reasoner_results_placeholder.tex | pending | True | ok | AeroGraph table slot must stay pending until all prompt-pack external-provider responses are valid-schema complete |
| pending_reasoner | docs/aerograph_nonmock_collection_plan.md | runbook | True | ok | External-provider reasoner collection gate |
...
| pending_reasoner | outputs/reports/live/aerograph_prompt_pack/aerograph_web_collection_checklist.csv | runbook | True | ok | Per-prompt AeroGraph external-provider collection checklist |
