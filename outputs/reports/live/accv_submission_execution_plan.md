...
| MarineCity neural 3D | pending | RGB/depth/pose, transforms, and point-cloud smoke are ready; neural metric rows are still zero. | Pending |
...
| --- | --- | --- | --- |
| 6/27 | System-level result consolidation | Re-run/refresh MarineCity real-Cesium capture + detector + reasoner smoke; verify actors/UAV cameras/classes; freeze 2D result bundle. | Updated simulation dashboard, smoke reports, Fig. 1/3 source package candidates |
...
- MarineCity system result: include only as a real-Cesium system smoke/protocol
  validation unless neural 3D and external reasoner gates complete.
...
| P1 | Neural 3D result | At least one real runner result or defensible reconstruction sanity result | Connect Nerfstudio/Instant-NGP/3DGS or import verified runner metrics |
...
- Do not claim completed neural 3D reconstruction until metric rows exist.
