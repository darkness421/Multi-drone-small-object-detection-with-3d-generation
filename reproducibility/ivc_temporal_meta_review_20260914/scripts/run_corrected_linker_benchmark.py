#!/usr/bin/env python3
"""Run corrected-channel temporal linker comparisons on frozen MMOT tracklets.

All methods consume the same versioned tracker outputs. Ground-truth identities
are read only after predictions and candidate links have been frozen, for
TrackEval metrics and link correctness auditing.
"""
...
def extract_descriptors(mmot, gt: dict, predictions: dict, image_map: dict[int, Path], model, device: torch.device, batch_size: int) -> tuple[dict[int, np.ndarray], dict]:
    observations = mmot.attach_npy_observations(gt, predictions, image_map)
    work = []
    for identity, rows in sorted(observations.items()):
        for row in mmot.BASE.sample_tracklet(rows):
            work.append((identity, row))
    ...
    with torch.inference_mode():
        for begin in range(0, len(work), batch_size):
            ...
            batch = torch.stack([
                corrected_crop(row["image_path"], row["bbox_xyxy"])
            ...
            output = output / norms
            for (identity, _), feature in zip(batch_items, output):
    ...
    return descriptors, {
        "tracklets_with_eligible_observations": len(observations),
        "sampled_crops": len(work),
...
def summarize_records(common, records: list[dict], families: list[str], trackers: list[str], methods: Iterable[str]) -> tuple[list[dict], list[dict]]:
    sequence_rows = []
    ...
    for tracker in trackers:
        for method in methods:
            ...
            for scope, selected in groups:
                ...
                if invalid:
                    for aggregation in ("pooled_detections", "equal_sequence_mean_metrics_total_counts"):
...
def main() -> int:
    ...
    parser.add_argument("--families", nargs="+", choices=("data28", "data30"), default=["data28", "data30"])
    ...
    parser.add_argument(
        "--deepoc-tracker-cache-dir", type=Path,
        help="Reuse immutable Deep OC-SORT JSONL caches from an earlier run.",
    )
            ...
            for class_id, class_name in enumerate(CLASS_NAMES):
                ...
                for tracker in args.trackers:
                    ...
                    if tracker in {"bytetrack", "ocsort"}:
                        cache = args.e0_dir / "results_raw/tracker_outputs" / family / sequence_dir.name / class_name / f"{tracker}.jsonl.gz"
                        predictions = load_predictions(cache)
                        tracker_source = str(cache)
                        tracker_source_hash = sha256(cache)
                    else:
                        relative_cache = Path("tracker_outputs") / family / sequence_dir.name / class_name / "deepocsort.jsonl.gz"
                        if args.deepoc_tracker_cache_dir is not None:
                            cache = args.deepoc_tracker_cache_dir / relative_cache
                            if not cache.is_file():
                                raise FileNotFoundError(f"missing requested Deep OC-SORT cache: {cache}")
                            predictions = load_predictions(cache)
                            runtime_rows.append({
                                **context, "method": "upstream_deepocsort",
                                "runtime_seconds": None,
                                "tracklet_count": len(prediction_tracklets(predictions)[0]),
                                "runtime_status": "reused_immutable_cache",
                            })
                        else:
                            cache = args.output_dir / relative_cache
                    ...
                    descriptors, descriptor_info = extract_descriptors(
                        mmot, gt, predictions, image_map, model, device, args.batch_size
                    )
                    descriptor_path = args.output_dir / "descriptors" / family / sequence_dir.name / class_name / f"{tracker}.npz"
    ...
    sequence_rows, aggregate_rows = summarize_records(mmot.COMMON, records, args.families, args.trackers, all_methods)
    link_summary = defaultdict(Counter)
    eligible_summary = defaultdict(int)
    for record in records:
        key = (record["family"], record["tracker"], record["method"])
    ...
    for key, counts in sorted(link_summary.items()):
        family, tracker, method = key
        known = counts["correct"] + counts["false"]
        link_summary_rows.append({
            "scope": family, "tracker": tracker, "method": method,
    ...
    for row in size_rows_raw:
        size_summary[(row["family"], row["tracker"], row["method"], row["pair_size_bin"])][row["posthoc_gt_correctness"]] += 1
    size_rows = []
    for key, counts in sorted(size_summary.items()):
        known = counts["correct"] + counts["false"]
        size_rows.append({
            "family": key[0], "tracker": key[1], "method": key[2], "size_bin": key[3],
            "accepted_correct": counts["correct"], "accepted_false": counts["false"], "accepted_unknown": counts["unknown"],
            "accepted_link_precision": counts["correct"] / known if known else None,
        })
    ...
    manifest = {
        ...
        "reid": reid_manifest,
        "aflink": af_manifest,
        "ground_truth_use": "oracle AABB inputs; identities only for metrics and posthoc link audit; historical IoU>=0.9 GT-geometry crop-admission retained",
        ...
        "deepoc_tracker_cache_dir": args.deepoc_tracker_cache_dir,
