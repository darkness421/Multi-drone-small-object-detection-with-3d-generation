#!/usr/bin/env python3
"""Generate descriptive link error-coverage curves from a frozen benchmark run.
...
def summarize_equal_sequence_only(bench, common, records: list[dict], families: list[str], trackers: list[str], methods: list[str]) -> tuple[list[dict], list[dict]]:
    """Combine classes per sequence, then report prespecified equal-sequence means.

    A full pooled identity matrix is unnecessary for the error--coverage audit
    and grows quadratically with the number of identities. The primary MMOT
    protocol already defines sequence means, so this sweep keeps that unit.
    """
    by_sequence = defaultdict(list)
    for record in records:
        key = (record["family"], record["sequence"], record["tracker"], record["method"])
        by_sequence[key].append(record)

    sequence_rows = []
    for (family, sequence, tracker, method), selected in sorted(by_sequence.items()):
        ...
        if invalid:
            ...
            continue
        metrics = bench.extended_metrics(
            common,
            bench.common_module_concatenate(common, [row["metric_data"] for row in selected]),
        )
    ...
    for tracker in trackers:
        for method in methods:
            for scope in (*families, "all_sequences"):
                selected = [
                ...
                totals = {
                    key: int(sum(row[key] for row in selected))
                    for key in ("IDSW", "FP", "FN", "TP", "Frag", "valid_gt_boxes", "predicted_boxes")
                }
                aggregate_rows.append({**base, **means, **totals})
...
def main() -> int:
        ...
        for sequence_dir in sorted(path for path in family_root.iterdir() if path.is_dir()):
            ...
            for class_id, class_name in enumerate(bench.CLASS_NAMES):
                ...
                for tracker in args.trackers:
                    ...
                    prediction_path = (
                        args.result_dir / "tracker_outputs" / family /
                        sequence_dir.name / class_name / f"{tracker}.jsonl.gz"
                    )
                    descriptor_path = (
                        args.result_dir / "descriptors" / family /
    ...
    sequence_rows, aggregate_rows = summarize_equal_sequence_only(
        bench, mmot.COMMON, records, list(family_roots), args.trackers, methods
    )
    decoded = {}
    ...
    link_summary = []
    for scope in (*family_roots, "all_sequences"):
        for tracker in args.trackers:
    ...
    manifest = {
        "status": "COMPLETE" if not invalid_outputs else "COMPLETE_WITH_INVALID_OUTPUT_VARIANTS",
        "claim_scope": "Descriptive error-coverage audit on frozen MMOT outputs; not threshold selection or calibration.",
        ...
        "trajectory_aggregation": "Per-sequence metrics and equal-sequence means; no pooled all-sequence identity matrix.",
        ...
        "inputs": {
            ...
            "frozen_result_manifest": {"path": str(args.result_dir / 'manifest.json'), "sha256": sha256(args.result_dir / 'manifest.json')},
        },
