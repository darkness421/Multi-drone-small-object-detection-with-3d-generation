...
def main() -> int:
    ...
    parser.add_argument("--tracker-cache-dir", type=Path)
    parser.add_argument("--descriptor-cache-dir", type=Path)
    parser.add_argument("--skip-pooled-aggregate", action="store_true")
    ...
    # BoxMOT emits one INFO block for every tracker construction. Keep the
    # experiment log compact without changing tracker behavior.
    ...
    for family, raw_root in args.family_root:
        ...
        for sequence_dir in sequences:
            print(f"processing {family}/{sequence_dir.name}", flush=True)
            ...
            for class_id, class_name in enumerate(bench.CLASS_NAMES):
                ...
                for tracker in args.trackers:
                    ...
                    tracker_root = args.tracker_cache_dir or args.output_dir
                    tracker_path = (
                        tracker_root / "tracker_outputs" / family / sequence_dir.name /
                        class_name / f"{tracker}.jsonl.gz"
                    )
                    if tracker_path.is_file():
                        predictions = bench.load_predictions(tracker_path)
                        invalid_detection_boxes = 0
                        tracker_seconds = None
                        tracker_runtime_status = "reused_immutable_cache"
                    else:
                        ...
                        tracker_runtime_status = "measured_and_cached"
                        bench.save_predictions(tracker_path, context, predictions)
                    descriptor_relative = (
                        Path("descriptors") / family / sequence_dir.name /
                    ...
                    if cached_descriptor_path is not None and cached_descriptor_path.is_file():
                        descriptors = bench.load_descriptors(cached_descriptor_path)
                    ...
                    for method in METHODS:
                        ...
                        runtime_rows.append({
                            **context, "method": method,
                            ...
                            "tracker_runtime_status": tracker_runtime_status,
    ...
    for row in records:
        for scope in (row["family"], "all_sequences"):
            key = (scope, row["tracker"], row["method"])
    ...
    manifest = {
        ...
        "tracker_cache_dir": str(args.tracker_cache_dir or args.output_dir),
        "descriptor_cache_dir": str(args.descriptor_cache_dir) if args.descriptor_cache_dir else None,
        "aggregate_protocol": (
