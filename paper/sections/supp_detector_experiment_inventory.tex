...
final VisDrone detector is fixed. The 640-pixel TinyPerson queue currently
contains YOLOv11l, YOLOv9c, YOLOv8l, YOLOv9m, SAFR-YOLO, and a P2P4-head-only
core-ablation row. As of the current snapshot, all 18 TinyPerson rows have
been collected: six detector
variants over seeds 42, 123, and 2026. These rows remain diagnostic because the
converted split is highly sparse (794 training images with 596 boxes and 786
validation images with 77 boxes), and all AP/AP50 values are close to zero.
The strongest TinyPerson row is YOLOv9m with AP/AP50 of approximately
0.0002/0.0010, while SAFR-YOLO reaches approximately 0.0000/0.0000. We
therefore keep TinyPerson as a supplementary domain-shift and dataset-conversion
limitation analysis rather than using it as a main detector claim. Related-work
TinyPerson rows are added only for models that are either cited in the
manuscript or explicitly added to the Related Work section with a matching
bibliography entry.
...
over seeds 42, 123, and 2026. AP, AP50, and F1 are scaled by $10^{-3}$ because
all tested models are near zero on the current converted sparse split; this
table is therefore used only as domain-shift evidence, not as a main comparison
claim.}
...
\begin{figure*}[t]
    ...
    \caption{TinyPerson 640-pixel supplementary stress-test summary. AP/AP50
    and F1 are shown at $10^{-3}$ scale because the converted sparse split
    produces near-zero AP for all tested models. This figure is included only
    as domain-shift evidence after the VisDrone detector claim is fixed.}
