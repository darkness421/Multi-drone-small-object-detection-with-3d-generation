...
\midrule
CSFPR-RTDETR [11] & VisDrone val, 1280, 3-seed & 0.2784 $\pm$ 0.0070 & 0.4431 $\pm$ 0.0094 & 0.5766 & 0.4292 & 0.4921 & 14.09 & staged related-work run \\
MFFSODNet [2] & VisDrone val, 1280, 3-seed & 0.0975 $\pm$ 0.0034 & 0.1830 $\pm$ 0.0070 & 0.6418 & 0.2465 & 0.3562 & 4.54 & scratch retrain; no pretrained checkpoint found \\
SFFEF-YOLO [4] & VisDrone val, 1280, 3-seed & 0.3749 $\pm$ 0.0040 & 0.5914 $\pm$ 0.0039 & 0.6540 & 0.5781 & 0.6137 & 20.87 & paper-faithful reimplementation \\
BPD-YOLO [7] & VisDrone val, 1280, 3-seed & 0.3804 $\pm$ 0.0035 & 0.5986 $\pm$ 0.0027 & 0.6626 & 0.5802 & 0.6187 & 24.53 & paper-faithful reimplementation \\
HF-D-FINE [12] & VisDrone val, 1280, 3-seed & 0.3619 $\pm$ 0.0129 & 0.5764 $\pm$ 0.0186 & 0.6685 & 0.5555 & 0.6067 & 25.48 & high-resolution/frequency-detail reproduction \\
UAVDet [16] & VisDrone val, 1280, 3-seed & 0.3812 $\pm$ 0.0013 & 0.6005 $\pm$ 0.0036 & 0.6542 & 0.5899 & 0.6203 & 33.55 & inspired reproduction; not official checkpoint \\
\bottomrule
\end{tabular}
\end{table*}
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\caption{Related-work coverage matrix. Models discussed in Related Work are not removed from the evaluation plan; each is marked as measured, runnable assets not staged, or citation-only. The live CSV version is stored in \texttt{paper/tables/related\_work\_coverage\_matrix.csv}.}
\label{tab:supp_related_work_coverage}
\begin{tabular}{llll}
\toprule
Model group & Examples & Status & Paper use \\
\midrule
Completed reproduction & CSFPR-RTDETR [11], MFFSODNet [2], SFFEF-YOLO [4], BPD-YOLO [7], HF-D-FINE [12], UAVDet [16] & normalized 1280 three-seed records complete & Main compact status table + supplementary protocol table \\
Cited prior; assets not staged & LRDS-YOLO [8] & compatible code/weights/adapter not staged & Coverage table and citation discussion \\
Citation-only/method motivation & DG-TSOD [6], D2A-Detector [10], UFO-DETR [14], CFIA [9], DFFormer [13], RT-UAV-SOD [15] & not fairly runnable today & Related-work motivation and coverage row \\
...
feature-activation heat-map comparison for YOLOv11l, YOLOv9c, and SAFR-YOLO on
...
\begin{figure*}[t]
    ...
    YOLOv9c, and SAFR-YOLO. The panels compare the spatial feature responses on
...
TinyPerson is treated as a detector-only supplementary stress test after the
final VisDrone detector is fixed. The completed 640-pixel TinyPerson queue
contains YOLOv11l, YOLOv9c, YOLOv8l, YOLOv9m, SAFR-YOLO, and a P2P4-head-only
core-ablation row. All 18 TinyPerson rows have been collected: six detector
variants over seeds 42, 123, and 2026. We additionally collect a
VisDrone-initialized SAFR-YOLO transfer row over the same three seeds. These
rows remain diagnostic because the converted split is highly sparse (794
training images with 596 boxes and 786 validation images with 77 boxes), and
all AP/AP50 values are close to zero. The strongest from-scratch TinyPerson row
is YOLOv9m with AP/AP50 of approximately 0.0002/0.0010. The VisDrone-initialized
SAFR-YOLO transfer row increases recall to approximately 0.0996 but still
reaches only approximately 0.00003/0.00012 AP/AP50. We therefore keep
TinyPerson as a supplementary domain-shift and dataset-conversion limitation
analysis rather than using it as a main detector claim. Related-work TinyPerson
rows are added only for models that are either cited in the manuscript or
explicitly added to the Related Work section with a matching bibliography entry.

\begin{table*}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{4pt}
\caption{TinyPerson 640-pixel supplementary stress test. Results are reported
over seeds 42, 123, and 2026. The SAFR-YOLO transfer row uses VisDrone-trained
SAFR-YOLO initialization before TinyPerson fine-tuning. AP, AP50, and F1 are
scaled by $10^{-3}$ because all tested models are near zero on the current
converted sparse split; this table is therefore used only as domain-shift
evidence, not as a main comparison claim.}
\label{tab:supp_tinyperson_640_stress}
\input{tables/tinyperson_640_stress_table}
\end{table*}

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.86\textwidth]{figures/results/paper_fig12_tinyperson_640_stress}
    \caption{TinyPerson 640-pixel supplementary stress-test summary, including
    the VisDrone-initialized SAFR-YOLO transfer diagnostic. AP/AP50 and F1 are
    shown at $10^{-3}$ scale because the converted sparse split produces
    near-zero AP for all tested models. This figure is included only as
    domain-shift evidence after the VisDrone detector claim is fixed.}
    \label{fig:supp_tinyperson_640_stress}
\end{figure*}
...
\scriptsize
\setlength{\tabcolsep}{4pt}
\caption{TinyPerson eval-only input-size sensitivity. Completed TinyPerson
checkpoints are evaluated at 640, 960, and 1280 pixels without additional
training. AP, AP50, and F1 are scaled by $10^{-3}$. This table is a
supplementary diagnostic for input-size sensitivity under the sparse converted
TinyPerson split, not a main detector claim.}
\label{tab:supp_tinyperson_eval_imgsz_sweep}
\input{tables/tinyperson_eval_imgsz_sweep_table}
\end{table*}

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.86\textwidth]{figures/results/paper_fig13_tinyperson_eval_imgsz_sweep}
    \caption{TinyPerson eval-only input-size sensitivity. The completed
    checkpoints are reused and only the evaluation image size is changed. On
    the current converted split, 640-pixel evaluation is the most stable for
    AP50, while the VisDrone-initialized SAFR-YOLO transfer row gives the
    highest recall.}
    \label{fig:supp_tinyperson_eval_imgsz_sweep}
\end{figure*}
