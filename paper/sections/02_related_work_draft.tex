...
Neural scene representations and 3D Gaussian-style renderers have made it
possible to reconstruct or render novel views from sparse image collections.
However, many reconstruction pipelines optimize appearance quality for the
scene as a whole, while UAV small-object perception needs object-centered
evidence under viewpoint ambiguity. In our setting, 3D is not used as a
standalone visual effect; it is used to ask whether additional viewpoints,
restored evidence, or approximate geometry can reduce class ambiguity for small
objects.
...
Nerfacto, Instant-NGP, and 3DGS-style runners can produce held-out smoke
...
evidence, but an unconstrained model should not directly control a UAV mission.
...
The current paper reports AeroGraph as a system interface and validation slot.
The reviewed prompt pack is schema-checked, but the headline external-provider
LLM/VLM replication is kept separate until provider outputs are imported and
validated. This conservative reporting avoids conflating deterministic
pipeline-validation logic with a final external-reasoner benchmark.
